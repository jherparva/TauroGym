...
export function SidebarProvider({
  ...
  // Cuando cambia a móvil, cerrar el sidebar
  React.useEffect(() => {
    if (isMobile) {
      setOpen(false)
    } else {
  ...
  // Leer el estado del sidebar desde una cookie al cargar
  React.useEffect(() => {
  ...
  // Guardar el estado del sidebar en una cookie cuando cambia
  React.useEffect(() => {
    if (typeof window !== "undefined" && !isMobile) {
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        tabIndex={-1}
        onClick={() => !isMobile && setOpen((open) => !open)}
